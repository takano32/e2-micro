...
## コスト構造(2026年6月請求の SKU 明細で確定)

- **無料**: VM 本体(月上限時間まで)・pd-standard 30GB — 無料枠で全額相殺を請求明細で確認済み
- **外部 IPv4**: 定価 $0.005/時だが、**現在はクレジットで全額相殺され実質 ¥0**(6月: Networking ¥345 → -¥345 → ¥0)。クレジットの正式名称・恒常性は未確認なので、**毎月 Networking 行が ¥0 のままか確認する**
- **実質課金は egress のみ**: Premium ティア、無料 1GB/月、超過 $0.12/GB
  - 6月実績: **28.24GiB = ¥521**(+移行スナップショット一時費 ¥34 で請求合計 ¥556)
  - 主因は claude / watcher の Anthropic API 送信。**コスト削減のレバーはこれだけ**(VM 上の `egress-report` が課金対象をそのまま計測)
  - 7月ペース約 50GiB/月 → 放置なら **¥900 前後/月** の見込み
